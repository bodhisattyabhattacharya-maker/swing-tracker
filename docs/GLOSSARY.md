...
| **Allowlist route** | `/api/cell-history`, the only endpoint that reads the database on demand. It validates `symbol` and `param` against a list derived from `COLUMNS` before either reaches a query, because the repo is public and the key is full-access. |
